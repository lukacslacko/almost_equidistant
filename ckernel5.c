...
/* choose best-conditioned 5-subset of neighbour indices (float det) */
static void best_quint(const V5 x, const V5 nb[], int k, int q[D]){
    ...
    double best=-1; int bq[D]={0,1,2,3,4};
    int idx[D];
    for (idx[0]=0; idx[0]<k-4; idx[0]++)
    for (idx[1]=idx[0]+1; idx[1]<k-3; idx[1]++)
    for (idx[2]=idx[1]+1; idx[2]<k-2; idx[2]++)
    for (idx[3]=idx[2]+1; idx[3]<k-1; idx[3]++)
    for (idx[4]=idx[3]+1; idx[4]<k; idx[4]++){
        double J[D][D];
        for (int r=0;r<D;r++) for (int c=0;c<D;c++)
            J[r][c] = 2*(xm[c]-mid(nb[idx[r]][c]));
        /* det via LU-lite */
        double m[D][D]; memcpy(m,J,sizeof m); double det=1;
        int sing=0;
        for (int cc=0;cc<D;cc++){
            int p=-1; double bv=0;
            for (int r=cc;r<D;r++){ double v=fabs(m[r][cc]); if (v>bv){bv=v;p=r;} }
            if (p<0||bv<1e-300){ sing=1; break; }
            if (p!=cc){ for(int j=0;j<D;j++){double t=m[cc][j];m[cc][j]=m[p][j];m[p][j]=t;} det=-det; }
            det*=m[cc][cc];
            for (int r=cc+1;r<D;r++){ double f=m[r][cc]/m[cc][cc];
                for (int j=cc;j<D;j++) m[r][j]-=f*m[cc][j]; }
        }
...
static int forced_coincident_c(Prob *P, State *st, int u, int v){
    ...
    if (nc < D+1) return 0;
    int idx[D+1];
    for (idx[0]=0; idx[0]<nc-5; idx[0]++)
    for (idx[1]=idx[0]+1; idx[1]<nc-4; idx[1]++)
    for (idx[2]=idx[1]+1; idx[2]<nc-3; idx[2]++)
    for (idx[3]=idx[2]+1; idx[3]<nc-2; idx[3]++)
    for (idx[4]=idx[3]+1; idx[4]<nc-1; idx[4]++)
    for (idx[5]=idx[4]+1; idx[5]<nc; idx[5]++){
        V5 rows[D];
        for (int r=0;r<D;r++)
...
static int noninjective_c(Prob *P, State *st, int v, const V5 x){
    for (int upi=0; upi<st->np; upi++){
        ...
        for (idx[4]=idx[3]+1; idx[4]<nc; idx[4]++){
            V5 nbv[D];
            for (int r=0;r<D;r++) memcpy(nbv[r], st->p[common[idx[r]]], sizeof(V5));
            Cand cd[2];
...
static void dfs(Prob *P, State *st, int oi, double cellw, int has_cellw){
    ...
    if (nnb >= D){
        /* try 5-subsets until well-conditioned */
        Cand cd[2]; int r=-1;
        int idx[D];
