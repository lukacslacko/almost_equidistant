#!/usr/bin/env python3
"""Controls for the exact dimension-six v5 residue boundary."""
...
import build_d6_current_residue_manifest_v5 as builder
import verify_d6_current_residue_manifest_v5 as checker


class CurrentResidueV5Controls(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.manifest = builder.build_manifest()
        cls.temporary = tempfile.TemporaryDirectory()
        cls.path = Path(cls.temporary.name) / "v5.json"
    ...
    def test_exact_counts_and_rejection_partition(self) -> None:
        ...
        self.assertEqual(k6["v4_count"], 805)
        self.assertEqual(k6["virtual_K7_rejection_count"], 49)
        self.assertEqual(k6["count"], 756)
        self.assertEqual(self.manifest["combined"]["count"], 911)
        self.assertFalse(
            set(k6["virtual_K7_rejected_indices"])
            & {record["index"] for record in k6["graphs"]}
        )

    def test_independent_checker_rebuilds_v5(self) -> None:
    ...
    def test_checker_imports_no_builder_or_k6_kernel(self) -> None:
        tree = ast.parse(
            (checker.ROOT / "verify_d6_current_residue_manifest_v5.py").read_text(
                encoding="utf-8"
            )
        )
        imports = []
        ...
        self.assertNotIn("build_d6_current_residue_manifest_v5", imports)
