#!/usr/bin/env python3
"""Independently audit a completed interval pass on the exact v5 residue.

The verifier reconstructs the selected graphs directly from the pinned v5
manifest (it does not import the v5 launch wrapper), validates every atomic
checkpoint and summary counter, and replays every claimed ``KILLED`` circle
slice through the frozen outward-rounded kernel.  ``ABORT`` and all other
non-KILLED states receive no mathematical credit.
"""
...
MANIFEST = "d6_current_residue_manifest_v5.json"
VERIFICATION = "d6_current_residue_manifest_v5_verification.json"
...
EXPECTED_SOURCES = {
    "wrapper": ("run_d6_interval_v5.py", "9328238b54dab51ae37c7ca18bca4fd47a99ed7bef4e1ece051ecb4228efb6a0"),
    "interval_runner": (
        "run_d6_interval_residue.py",
        "58fbbbc548ec3cc6b9d5b231a6790249b02a75d7bfd82f1afcd7fc77fce59404",
    ),
    "cdriver6.py": ("cdriver6.py", "8f65948c8bc95242d0fde50a8cbcb798cb0e225b78a8ce626382d00135b95173"),
    "ckernel6.c": ("ckernel6.c", "383ef7a17c328869c338f64a16f2b90875ff968fe2fc20e9a5c3094c830e0874"),
    "ival.py": ("ival.py", "524e41e0d0637a5352c59ec998009b968f2c9f5e7f31b7f7927e8bc36c2e6fa0"),
}
...
def reconstruct_v5(configuration: dict) -> list[dict]:
    ...
    require(manifest.get("status") == "COMPLETE_EXACT_FILTER_UNION", "manifest status")
    require(manifest.get("class_order") == ["K7", "K6_only"], "class order")
    require(verification.get("status") == "PASS", "verification status")
    require(
        verification.get("manifest", {}).get("sha256") == EXPECTED_MANIFEST_SHA256,
        "verification binding",
    )

    selection_layer = configuration["selection"]["selection_layer"]
    require(selection_layer.get("kind") == "exact_d6_current_residue_v5", "selection kind")
    require(
        selection_layer.get("manifest", {}).get("sha256") == EXPECTED_MANIFEST_SHA256,
    ...
    require(selection_layer.get("combined") == 911, "selection base count")
    require(selection_layer.get("counts") == EXPECTED_COUNTS, "selection class counts")
    ...
    for class_name, population in (("K7", "K7"), ("K6_only", "K6")):
        ...
        for record in records:
            ...
            graphs.append({"index": index, "population": population, "adjacency": adjacency})
            ordered_indices.append(index)
    require(len(graphs) == 911, "base graph count")
    require(
        object_sha256(ordered_indices) == manifest["combined"]["ordered_indices_sha256"],
        "base order hash",
    )

    sampling = configuration["sampling"]
    require(sampling["population"] in ("all", "K7", "K6"), "population selector")
    if sampling["population"] != "all":
        graphs = [graph for graph in graphs if graph["population"] == sampling["population"]]
    ...
    graphs = [graph for position, graph in enumerate(graphs) if position % shards == shard]
    ...
    require(
        selection["indices_sha256"] == object_sha256([graph["index"] for graph in graphs]),
        "selected index hash",
    )
    require(
...
def validate_source_boundary(configuration: dict) -> dict:
    require(file_sha256(ROOT / "verify_d6_interval_benchmarks.py") == EXPECTED_COMMON_SHA256,
    ...
    require(sources.get(cdriver6.LIBNAME) == file_sha256(ROOT / cdriver6.LIBNAME),
            "kernel binary hash")
    ...
    for _key, (filename, expected) in EXPECTED_SOURCES.items():
        ...
        require(hashlib.sha256(blob).hexdigest() == expected, f"commit blob: {filename}")
...
def verify_report(report_path: Path, expected_hash: str | None, replay: bool) -> dict:
    ...
    require(configuration.get("schema") == 3, "configuration schema")
    ...
    require("caffeinate" in configuration["launch"]["outer_launch_command"],
            "sleep prevention not recorded")
    graphs = reconstruct_v5(configuration)
    require(report["total_graphs"] == report["completed_graphs"] == len(graphs), "coverage")
    require(report["session"]["workers"] == 11, "worker count")
    require(len(report["results"]) == len(graphs), "result count")
    provenance = validate_source_boundary(configuration)
    control_audit = common.validate_controls(configuration["kernel_controls"], replay=replay)
    ...
    require(report["unresolved_total"] == len(graphs) - statuses["KILLED"], "residue count")
    ...
    for result, graph in zip(report["results"], graphs, strict=True):
        ...
        if replay:
            for record in result["winning_slice_records"]:
                ...
                expected = (record["status"], record["nodes"], record["unresolved_cells"])
    ...
    return {
        "schema": "d6-interval-v5-verification-v1",
        "status": "PASS",
        "claim": (
            "Only the listed KILLED graphs are certified non-realizable by the "
            "outward-rounded interval kernel. All other statuses remain unresolved."
        ),
