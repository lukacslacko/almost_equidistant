...
set in $\R^5$:\\ $f(5)\le 18$, toward the exact value\\[4pt]
\large A computer-assisted campaign, with certified non-realizability at
$19$ and $20$ points}
\author{}
\date{August 5, 2026 (snapshot; campaign in progress)}
...
\begin{result}
$f(5) \le 18$. More precisely: there is no almost-equidistant set of $19$
points in $\R^5$ (and none of $20$ points), by certified elimination of all
candidate unit-distance graphs. Combined with the Larman--Rogers set,
$16 \le f(5) \le 18$.
\end{result}

\begin{wip}
The two remaining levels are being processed by the same pipeline:
all $8825$ candidate graphs on $18$ vertices (certification under way;
partial results in \texttt{results\_n18.json}), then all $12654$ on $17$
vertices. Independent numerical optimization over the $17$-vertex
candidates (Levenberg--Marquardt, $300$ restarts per graph) has so far
found no graph anywhere near realizable --- the smallest residual over the
first ${\sim}5000$ graphs is $\approx 0.5$, against $10^{-30}$ for
realizable controls. If every $17$-vertex candidate is eliminated,
$f(5)=16$; a certified survivor would instead pin $f(5)$ at $17$ or $18$.
\end{wip}
...
\texttt{results\_n20.json} and \texttt{results\_n19.json}. Numerical
corroboration: Levenberg--Marquardt over all $8$ graphs at $n=20$ and all
$340$ at $n=19$ finds best residuals $\ge 2.6$ (distinct-point solutions),
i.e.\ nowhere near realizable.
