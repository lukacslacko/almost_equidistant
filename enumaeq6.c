/* Independent enumeration of ALL abstract almost-equidistant graphs in R^6
 ...
 *
 * Single file, no dependencies:  cc -O2 -o enumaeq6 enumaeq6.c
 * Usage: enumaeq6 [maxn]      (default maxn = 13)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* K_{1,3,3,3} subgraph: disjoint triples T1,T2,T3 with all 27 cross
...
int main(int argc, char **argv){
    ...
    while (n < maxn && lvl_len > 0){
        ...
        if (n >= 12){
            char fn[64]; snprintf(fn, sizeof fn, "aeq_d6_n%d.bin", n);
            FILE *f = fopen(fn, "wb");
            if (f){
