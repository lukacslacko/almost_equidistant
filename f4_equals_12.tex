...
mirror-image unit cross-polytopes. Two independent implementations (a Python
reference and a C production version, roughly $1000\times$ faster) produce
identical search trees node-for-node on matched instances. A runtime canary
aborts the computation if a parametrized circle point ever violates its own
defining constraint; during development this caught a genuine bug, after which
...
\emph{(Reference implementation \texttt{decide.py} + \texttt{ival.py};
production port \texttt{ckernel.c}. The two produce identical search trees.)}
...
vanishing affine volumes). Rather than analyse the channel, the driver switches
decomposition: a different elimination order re-parametrizes the geometry, the
degenerate family no longer sits at a tangency of the new parametrization, and
the standard machinery certifies all $24$ slices of the full circle directly.
The final certificate for that graph rests entirely on the alternate
decomposition and is self-contained.

\subsection{Reproduction}
The package provides: \texttt{enumerate\_aeq.py} (Appendix~A; also verifies
all published counts), \texttt{ival.py}/\texttt{decide.py} (reference engine),
\texttt{ckernel.c}/\texttt{cdriver.py} (fast engine), the graph list
\texttt{aeq\_d4\_n13.json}, and \texttt{reproduce.py}, which runs the two
validation controls (Lemma-12 graph killed; cross-polytope graph preserved)
and then certifies all $59$ graphs by the strategy above. On a 12-core machine
the full run takes on the order of half a day; the computation is
deterministic, so identical hardware reproduces identical search trees.
